import { d as now, l as loop, c as create_ssr_component, e as escape, f as each, h as createEventDispatcher, i as add_attribute, v as validate_component, b as subscribe, n as noop, o as onMount, j as onDestroy } from "../../../chunks/index.js";
import { w as writable } from "../../../chunks/index3.js";
import { c as createGQLWSClient, b as createSubscription } from "../../../chunks/graphql-ws.js";
...
const TopTicks = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let $observable, $$unsubscribe_observable;
  let { observable } = $$props;
  $$unsubscribe_observable = subscribe(observable, (value) => $observable = value);
  if ($$props.observable === void 0 && $$bindings.observable && observable !== void 0)
    $$bindings.observable(observable);
  $$unsubscribe_observable();
  return `${$observable ? `<h3>Top Ticks</h3>
<table><thead><tr><th>id</th>
            <th>focus group</th>
            <th>user</th>
            <th>rating</th>
            <th>tick ts</th></tr></thead>
    <tbody>${each($observable.data.ratingtick, (item) => {
    return `<tr><td>${escape(item.id)}</td>
            <td>${escape(item.focusgroup)}</td>
            <td>${escape(item.username)}</td>
            <td>${escape(item.rating)}</td>
            <td>${escape(item.tick_ts)}</td>
        </tr>`;
  })}</tbody></table>` : ``}`;
});
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  onMount(async () => {
    gqlwsClient = createGQLWSClient({}.VITE_HASURA_GRAPHQL_URL);
  ...
  ratingtick(order_by: {id: desc}, limit: $limit) {
    ...
    const rxjsobservable = createSubscription(gqlwsClient, gql, variables);
...
<form><label for="${"focusgroup"}">focus group: </label><input type="${"text"}" name="${"focusgroup"}"${add_attribute("value", focusGroupName, 0)}>
	<label for="${"username"}">username: </label><input type="${"text"}" name="${"focusgroup"}"${add_attribute("value", userName, 0)}>
	...
	<div>slider Value: ${escape(sliderValues[0])}</div>

	<button ${""}>Start</button>
	<button ${"disabled"}>Stop</button></form>
<div>Tick output: ${escape(tickLog)}</div>

${validate_component(TopTicks, "TopTicks").$$render($$result, { observable: gqlwsObservable }, {}, {})}

<button>Delete most records</button>

<form action="${"/api/cleanup.json"}" method="${"post"}"><input type="${"submit"}" value="${"Delete most records via POST"}"></form>

<div><a href="${"/"}">Home</a></div>

<div>({}).VITE_HASURA_GRAPHQL_URL ${escape({}.VITE_HASURA_GRAPHQL_URL)}</div>`;
  } while (!$$settled);
  return $$rendered;
});
export {
